...
export default class Socket<T, RT> extends Heart {
    ...
    reconnectTimer: NodeJS.Timeout|null = null // 重连计时器
    ...
    constructor(ops: Ioptions<RT>) {
        super()
        ops.url = import.meta.env.VITE_APP_GLOB_WEBSOCKET_API + ops.url
    ...
    onopen(callback: Callback): void {
        this.ws.onopen = event => {
            clearTimeout(this.reconnectTimer as NodeJS.Timeout) // 清除重连定时器
    ...
    onmessage(callback: MessageCallback<RT>): void {
        this.ws.onmessage = (event: MessageEvent<string>) => {
            ...
            const { code, data } = JSON.parse(strMessage)
            if (code === 0) {
                // 收到任何消息，重新开始倒计时心跳检测
                super.reset()
                super.start(() => {
